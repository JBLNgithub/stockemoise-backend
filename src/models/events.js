...
export const nextEvents = async(SQLClient) => {
    const query = `SELECT n.id, n.title, n.cover, en.dateEvent AS "dateEvent", 'eventNews' AS type FROM news n INNER JOIN eventNews en on (n.id = en.id AND en.dateEvent > ${sqlAdapter.NOW()}) UNION SELECT c.id, c.title, c.cover, c.dateEvent AS "dateEvent", 'concert' AS type FROM concert c WHERE c.dateEvent > ${sqlAdapter.NOW()} ORDER BY "dateEvent" ASC`
